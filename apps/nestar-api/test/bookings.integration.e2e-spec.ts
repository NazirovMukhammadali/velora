...
describeIntegration('Bookings GraphQL (integration, mongo-memory)', () => {
    ...
    it('creates tour booking and reports purchase eligibility', async () => {
        ...
        expect(createResponse.body.data.createTourBooking.bookingStatus).toBe(BookingStatus.CONFIRMED);

        const checkQuery = `
          query {
            hasConfirmedOrPaidTourBooking(agentId: "${authAgentId.toString()}")
          }
        `;
        ...
        expect(checkResponse.body.errors).toBeUndefined();
        expect(checkResponse.body.data.hasConfirmedOrPaidTourBooking).toBe(true);
    });
});
