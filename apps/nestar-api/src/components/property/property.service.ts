...
export class PropertyService {
    ...
    public async getProperty(memberId: Types.ObjectId | null, propertyId: Types.ObjectId): Promise<Property> {
        ...
        if (memberId) {
            ...
            const likeInput = {
                ...
                likeRefId: propertyId as unknown as Types.ObjectId,
    ...
    public async getProperties(memberId: Types.ObjectId, input: PropertiesInquiry): Promise<Properties> {
        ...
        const sort: T = { [input?.sort ?? 'createdAt']: input?.direction ?? Direction.DESC }; // dynamic key +1
    ...
    public async getFavorites(memberId: Types.ObjectId, input: OrdinaryInquiry): Promise<Properties> {
        return await this.likeService.getFavoriteProperties(memberId as unknown as any, input);
    }
