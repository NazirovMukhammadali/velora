...
export class CommentService {
	...
	public async createComment(memberId: Types.ObjectId, input: CommentInput): Promise<Comment> {
		...
		const hasEligiblePurchase = await this.bookingsService.hasConfirmedTourBooking(
			memberId,
	...
	public async getAgentReviewStats(
		...
		if (memberId) {
			const writer = await this.memberService.getMember(null, shapeIntoMongoObjectId(memberId));
			if (writer.memberType === MemberType.USER && writer.memberStatus === MemberStatus.ACTIVE) {
				canWriteReview = await this.bookingsService.hasConfirmedTourBooking(memberId, String(agentId));
			}
		}
