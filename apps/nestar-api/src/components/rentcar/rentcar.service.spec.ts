...
describe('RentcarService', () => {
    ...
    it('returns empty-safe rentcar list', async () => {
        ...
        const result = await service.getRentcars({ page: 1, limit: 10, search: {} } as any);
    ...
    it('delegates favorite rentcars query', async () => {
        ...
        const result = await service.getFavoriteRentcars(new Types.ObjectId(), { page: 1, limit: 10 } as any);
    ...
    it('updates rentcar by admin', async () => {
        ...
        const result = await service.updateRentcarByAdmin({ _id: updated._id, carTitle: 'Updated' } as any);
