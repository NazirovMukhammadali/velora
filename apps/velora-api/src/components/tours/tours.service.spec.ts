...
describe('ToursService', () => {
	...
	it('updates owned tour only', async () => {
		...
		const result = await service.updateTour(memberId, { _id: updated._id, tourTitle: 'Updated' });
	...
	it('soft deletes owned tour', async () => {
		...
		const result = await service.removeTour(memberId, removed._id);
	...
	it('delegates favorite tours query to LikeService', async () => {
		...
		const result = await service.getFavoriteTours(memberId, { page: 1, limit: 10 });
	...
	it('returns popular tours sorted query result', async () => {
		...
		const result = await service.getPopularTours({ page: 1, limit: 8 });
	...
	it('soft deletes tour by admin', async () => {
		...
		const result = await service.removeTourByAdmin(deleted._id);
