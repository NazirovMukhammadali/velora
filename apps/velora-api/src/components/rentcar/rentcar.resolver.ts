...
export class RentcarResolver {
    ...
    @Mutation(() => Rentcar)
    public async createRentcar(
        @Args('input') input: RentcarInput,
    ): Promise<Rentcar> {
        return await this.rentcarService.createRentcar(input);
    }

    @Query(() => Rentcars)
    public async getRentcars(
        @Args('input') input: RentcarsInquiry,
    ): Promise<Rentcars> {
    ...
    @UseGuards(RolesGuard)
    @Query(() => Rentcars)
    public async getAllRentcarsByAdmin(
        @Args('input') input: AllRentcarsInquiry,
    ): Promise<Rentcars> {
    ...
    @UseGuards(RolesGuard)
    @Mutation(() => Rentcar)
    public async updateRentcarByAdmin(
        @Args('input') input: RentcarUpdate,
    ): Promise<Rentcar> {
    ...
    @UseGuards(RolesGuard)
    @Mutation(() => Rentcar)
    public async removeRentcarByAdmin(
        @Args('rentcarId', { type: () => ID }) rentcarId: string,
    ): Promise<Rentcar> {
