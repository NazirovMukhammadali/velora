...
export class HotelsResolver {
	...
	@Mutation(() => Hotel)
	public async createHotel(@Args('input') input: HotelInput): Promise<Hotel> {
		return await this.hotelsService.createHotel(input);
	}

	@Query(() => Hotels)
	public async getHotels(@Args('input') input: HotelsInquiry): Promise<Hotels> {
	...
	@UseGuards(RolesGuard)
	@Query(() => Hotels)
	public async getAllHotelsByAdmin(@Args('input') input: AllHotelsInquiry): Promise<Hotels> {
	...
	@UseGuards(RolesGuard)
	@Mutation(() => Hotel)
	public async updateHotelByAdmin(@Args('input') input: HotelUpdate): Promise<Hotel> {
	...
	@UseGuards(RolesGuard)
	@Mutation(() => Hotel)
	public async removeHotelByAdmin(@Args('hotelId', { type: () => ID }) hotelId: string): Promise<Hotel> {
