...
export class FlightsService {
		...
		if (memberId) {
			...
			if (newView) {
				await this.flightModel
					.findByIdAndUpdate(shapeIntoMongoObjectId(flightId), { $inc: { flightViews: 1 } }, { new: true })
					.exec();
	...
	public async likeTargetFlight(memberId: Types.ObjectId, flightId: Types.ObjectId): Promise<Flight> {
		...
		const updated = await this.flightModel
			.findByIdAndUpdate(flightId, { $inc: { flightLikes: modifier } }, { new: true })
			.exec();
	...
	public async updateFlightByAdmin(input: FlightUpdate): Promise<Flight> {
		...
		const updated = await this.flightModel
			.findOneAndUpdate({ _id: input._id, flightStatus: { $ne: FlightStatus.DELETE } }, input, { new: true })
			.exec();
